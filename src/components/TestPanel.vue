...
const createColumns = (rawData: string[][]) => {
  ...
  //宽度设置为60/60/60/60/120/120
  for (let item of rawData[0]) {
    cols.push({ title: item, key: item, width: 60 })
  }
  //愚蠢的写法，但是为了保证宽度正确，在更换参数数量时这里会变动，目前似乎只有通过统一数据列数量来解决  
  cols[1].width = 
  cols[2].width = 60
  cols[3].width = 45
  cols[4].width = 180
...
function handleTesting() {
  if (fileListLength.value) {
    ...
    //console.log(rawFile)
    //上述输出csv文件内容
    console.log(`/testUsecases/${props.context}/${usecaseType.value}.csv`)
    //上述输出/testUsecases/calendarProblem/basic-boundary.csv
    if (!rawFile) {
  ...
  console.log('测试结果为',result.value)
      ...
      <n-tabs v-model:value="currTab">
        ...
        >
          ...
          >
            ...
            <p class="subtitle">
              <slot name="subtitle-description">
                问题描述
              </slot>
            </p>

            <p class="text-body">
              <slot name="question-detail">
                先这样，再那样
              </slot>
            </p>

            <p class="subtitle">
              <slot name="subtitle-agorithm">
                算法思想
              </slot>
            </p>

            <p class="text-body">
              <slot name="agorithm-detail">
                先这样，再那样
              </slot>
            </p>
